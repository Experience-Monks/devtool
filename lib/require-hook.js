module.exports = function requireHook (opts) {
  ...
  // process.env.NODE_PATH and require resolves
  // are not correct in Electron unless this is called from
  // the renderer, for some reason. Needs more investigating!
  // However, this will break requires for 'native-module' and such...
  // Module._initPaths();
  ...
  require.extensions['.js'] = function devtoolCompileModule (module, file) {
    ...
    try {
      ...
      // Determine file and line/column of the syntax
      // error.
      ...
      if (realErr && realErr.annotated) {
        ...
        // Add file to message for better debugging.
        // This might occur e.g. if we have a ES6 syntax feature
        // that is supported by acorn, but not Node/V8.
        err.message = '\n' + file + '\n' + err.message;
      }
